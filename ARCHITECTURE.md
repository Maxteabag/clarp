...
| Backends | `backend/` (one `Backend` strategy class per CLI: `base.py`, `stream_json.py`, `claude.py`, `codex.py`, `agy.py`, `grok.py`, `opencode.py`, `deepseek.py`, `registry.py`; contract in [docs/architecture/backend-strategy.md](docs/architecture/backend-strategy.md)), `backends.py` (facade: id constants, `normalize()`, `by_id()` / `for_agent()`, the transitional `BackendAdapter` rows), `clarp_runner.py` (Claude), `codex_runner.py` + `codex_app_server.py`, `agy_runner.py`, `grok_runner.py`, `opencode_runner.py`, `runner_common.py`, `provider_capabilities.py` (model catalogue), `backend_auth.py`, `backend_usage.py` |
