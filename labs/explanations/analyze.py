...
def analyze(directory):
    ...
    for path in sorted(directory.glob("*.jsonl")):
        for line in path.read_text().splitlines():
            ...
            rows.append(row)
            groups[(row["transport"], row["prompt"], row["batch_size"], row.get("detail_level", 3), row.get("prompt_sha256", "legacy"), row.get("effort", "low"))].append(row)
    result = {"recorded_model_calls": len(rows), "format_valid": sum(bool(r.get("valid")) for r in rows), "conditions": []}
    for (transport, prompt, size, level, prompt_hash, effort), samples in groups.items():
        ...
        result["conditions"].append({"transport": transport, "prompt": prompt, "batch_size": size, "detail_level": level, "prompt_sha256": prompt_hash, "effort": effort,
