...
|---|---|---|---|
| 15 | 1 | local candidate, 2026-09-26 | Sub-agents and background processes are counted apart. `/agents/snapshot` `background_jobs.sub_agents` is now the number of running helper agents (equal to `running_children`) and `background_jobs.count` the number of background processes, excluding the job that only mirrors a helper; before, three helpers read as `{"count": 3, "sub_agents": 3}` and "6 sub-agents running" could appear, now `{"count": 0, "sub_agents": 3}`. `status_text` no longer repeats a count: it describes activity ("slice5-helper: running tests", "3 working, 1 waiting", a process's progress line or title), with a bare count only as a last resort. `sub_agents` may now exceed `count`. New `GET /background-jobs/<job_id>` returns the job with its `timeline`, `progress`, `log` tail (64 KB, path-checked) and owner links (`owner_session`, `owner_agent_id`, `started_trace_id`, `helper_session`). Job rows gain `progress_text`, `progress_at`, `log_path`, `worker_cwd`, `started_trace_id`; a job whose worker PID died now fails at once (`worker_vanished`). Feature `background_process_detail`. Older clients keep working but show the new counts. |
