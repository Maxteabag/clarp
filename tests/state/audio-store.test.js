...
vi.mock('../../web/src/stores/app.svelte.js', () => ({
  app: { session: 'rachel' }, flash: vi.fn(), logState: vi.fn(),
}));
...
vi.mock('@core/audio-faults.js', () => ({ createFaultMonitor: () => ({}) }));
...
describe('audio element unlocking', () => {
  it('ignores a second unlock while play is pending and restores the real mute state', async () => {
    const gate = deferred();
    const player = {
      src: 'https://clarp.test/voice.mp3', muted: false, currentTime: 4,
      play: vi.fn(() => gate.promise), pause: vi.fn(),
    };
