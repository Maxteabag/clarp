...
def test_contact_avatar_is_retained_while_agent_references_it(tmp_path):
    ...
    personas.create(name="Custom Nova", voice_id='{"cartesia":"voice-1"}')
    persona = personas.get("Custom Nova")
    agent_id = agents.create_agent(
        persona="Custom Nova", voice_id="", cwd="/tmp", session="nova")
...
def test_custom_contact_can_be_edited_without_touching_its_chat(tmp_path):
    personas.create(
        name="Custom Nova", voice_id='{"deepgram":"flux-haley-en"}',
        personality="Original")
    agent_id = agents.create_agent(
        persona="Custom Nova", voice_id='{"deepgram":"flux-haley-en"}',
        cwd=str(tmp_path), session="nova-chat")

    updated = personas.update(
        original_name="Custom Nova", name="Nova Prime",
