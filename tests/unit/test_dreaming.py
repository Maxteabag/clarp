...
def test_completed_dream_removes_its_disposable_worktree(tmp_path, monkeypatch):
    from lib import launch_paths

    aid = _agent("dream-cleanup")
    agent = agents_db.get_by_agent_id(aid)
    run = dreaming.create_dream_run(
        agent,
        local_date="2026-09-04",
    ...
    dream_round = dreaming.list_dream_runs(session="dream-cleanup")[0]["rounds"][0]
    monkeypatch.setattr(launch_paths, "workspace_root", lambda: tmp_path)
    worktree = tmp_path / ".clarp/dream-worktrees" / run["run_id"]
