...
def core_server(tmp_path):
    """FakeClaude-seeded DB + real server in-process. The turn is driven by
    the caller AFTER boot so broadcasts land on live SSE subscribers."""
    home = tmp_path / "home"
    home.mkdir()
    ...
    # Backend codex: INV2 ghost-session reconcile (bound Claude session must
    # have a transcript file) only applies to claude backends. The core
    # protocol is backend-agnostic; codex keeps the binding the test drives.
...
def test_sse_replay_and_event_shapes(core_server):
    ...
    assert replay.events[-1][1].get("type") == "agent-roster"
