...
# tests that reach them through this module (tests patch `_write`).
_json = store.encode
_decode = store.decode
_write = store.write
...
def _builtin_role(c, agent_id: str) -> str | None:
    return store.builtin_role(agent_id, c)
...
def trigger_definitions() -> list[dict]:
    return [{"trigger_id": r["trigger_id"], "version": r["version"],
            ...
            for r in store.trigger_definition_rows()]
...
def _config(c, agent_id: str):
    row = store.config_row(agent_id, c)
    if not row:
...
def _save_attachments(c, agent_id: str, values: list[dict], now: int):
    store.save_attachments(c, agent_id, values, now)
...
def create(session: str, template_id: str = "task-labels", scope=None, attachments=None, options=_UNSET, execution=None) -> dict:
    ...
    with _write() as c:
        a = _agent(a["agent_id"])
        existing = store.has_config(a["agent_id"], c)
...
def _pending_demand_claim(c, agent_id: str) -> bool:
    return store.pending_demand_claim(agent_id, db.now_ms(), c)
...
def get(session: str, *, include_runtime: bool = True) -> dict:
    ...
    current = store.active_run_id(a["agent_id"], c)
...
def list_janitors(*, include_runtime: bool = True) -> list[dict]:
    return [get(agent_id, include_runtime=include_runtime) for agent_id in store.janitor_agent_ids()]
...
def configure(session: str, expected_revision: int, *, template_id=None, scope=None, attachments=None, model=None, effort=None, execution=None, backend=None, options=_UNSET) -> dict:
    ...
    with _write() as c:
        ...
        store.replace_config_settings(c, a["agent_id"], template_id=selected_template, scope=selected_scope,
                                      execution=selected_execution, options=selected_options, now=now)
        if selected_scope != _decode(row["scope_json"], {}) or selected_template != row["template_id"]:
            store.clear_progress_for_agent(c, a["agent_id"])
...
def adopt_options(session: str, expected_revision: int, options: dict) -> dict:
    ...
    with _write() as c:
        ...
        now = db.now_ms()
        _fence(c, a["agent_id"], now)
        store.replace_config_options(c, a["agent_id"], _option_patch("tool-explainer", options, stored), now)
    return get(session, include_runtime=False)
...
def set_enabled(session: str, expected_revision: int, enabled: bool) -> dict:
    ...
    with _write() as c:
        ...
        if enabled:
            ...
            if not store.has_enabled_attachment(a["agent_id"], c):
        ...
        _fence(c, a["agent_id"], now)
        store.set_config_enabled(c, a["agent_id"], enabled, now)
    return get(session)
...
def remove(session: str, expected_revision: int) -> bool:
    ...
    with _write() as c:
        ...
        now = db.now_ms()
        _fence(c, a["agent_id"], now)
        store.bump_config(c, a["agent_id"], now, disable=True)
...
def _handoff_labels(c, source: dict, source_config, successor_session: str, successor_revision: int, now: int, runtime_busy: dict) -> dict:
    ...
    targets = [row["agent_id"] for row in store.owned_target_rows(source["agent_id"], c)
    ...
    store.transfer_label_ownership(c, successor["agent_id"], targets, now)
...
def _active_attachment(c, attachment_id: str, generation: int):
    row = store.attachment_with_config(attachment_id, c)
...
def get_progress(attachment_id: str) -> dict:
    row = store.progress_row(attachment_id)
...
def _save_progress(c, attachment_id, generation, state, next_run_at=None):
    ...
    store.save_progress(c, attachment_id, generation, state, db.now_ms())
    if next_run_at is not None or "next_run_at" in state:
        store.set_next_run(c, attachment_id, next_run_at)
...
def create_run(attachment_id: str, generation: int, candidates: list[dict], run_id: str | None = None, progress: dict | None = None) -> dict:
    ...
    with _write() as c:
        ...
        store.insert_run(c, run_id=run_id, agent_id=attachment["agent_id"], session=attachment["session"],
                         attachment_id=attachment_id, generation=generation, trace_id=run_id,
                         candidates=candidates, configuration=frozen, created_at=db.now_ms())
...
def _public_run(row) -> dict:
    ...
    demand = store.demand_result_json(row["run_id"])
    value["demand_result"] = _decode(demand, {}) if demand is not None else None
    value["results"] = [{"target_agent_id": r["target_agent_id"], "target_session": r["target_session"],
                        ...
                        for r in store.effect_rows(row["run_id"])]
    return value
...
def get_run_for_trace(trace_id: str) -> dict | None:
    row = store.run_row_for_trace(trace_id)
    return _public_run(row) if row else None


def list_runs(session: str, limit: int = 30) -> list[dict]:
    a = _agent(session)
    return [_public_run(r) for r in store.run_rows_for_agent(a["agent_id"], max(1, min(int(limit), 100)))]


def active_runs(agent_id: str = "") -> list[dict]:
    return [_public_run(r) for r in store.active_run_rows(agent_id)]


def has_active_run(agent_id: str) -> bool:
    return store.has_active_run(agent_id)


def _active_run(c, run_id):
    row = store.run_row(run_id, c)
    if not row:
    ...
    try:
        ...
        if row["status"] == "queued" and attachment["trigger_id"] == "active-interval":
            ...
            if not application_activity.active(_decode(attachment["config_json"], {})["idle_timeout_seconds"]):
                store.cancel_run(db.conn(), run_id, db.now_ms(), error="Application became inactive before execution",
                                 statuses=("queued",))
                return False
        return True
    except JanitorError:
...
def mark_started(run_id: str) -> dict:
    with _write() as c:
        ...
        store.mark_run_started(c, run_id, db.now_ms())
    return get_run(run_id)
...
def finish_run(run_id: str, outcome: str = "", error: str = "") -> dict:
    with _write() as c:
        ...
        if outcome == "changed" and "changed" not in receipts:
        ...
        store.finish_run_row(c, run_id, status="failed" if outcome == "error" else "cancelled" if outcome == "cancelled" else "completed",
                             outcome=outcome, now=now, error=safe_error)
        store.record_config_run(c, row["agent_id"], now, safe_error)
    return get_run(run_id)


def owned_label(agent_id: str, target_session: str) -> str | None:
    row = store.owned_label_row(agent_id, target_session)
...
def _task_signatures(c, agent_ids: list[str]) -> dict[str, str]:
    ...
    for offset in range(0, len(agent_ids), 500):
        for row in store.task_signature_rows(c, agent_ids[offset:offset + 500]):
...
def visible_labels() -> dict[str, str]:
    c = db.conn()
    now = db.now_ms()
    rows = store.label_ownership_with_status(c)
...
def review(run_id: str, target_session: str, observed_state_id: int, outcome: str, label: str | None = None, reason: str = "") -> dict:
    ...
    with _write() as c:
        ...
        existing = store.effect_row(run_id, target["agent_id"], c)
        if existing:
        ...
        queued = store.has_queued_turn(target["agent_id"], c)
        ...
        ownership = store.label_ownership_row(target["agent_id"], c)
        ...
        if outcome == "changed":
            ...
            store.record_config_change(c, run["agent_id"], now)
        elif outcome == "same_task" and owned:
            store.refresh_label_ownership(c, target["agent_id"], task_signature=_task_signature(c, target["agent_id"]),
                                          valid_until=now + LABEL_MAX_AGE_MS, now=now)
        store.insert_effect(c, run_id=run_id, target_agent_id=target["agent_id"], target_session=target_session,
                            observed_state_id=observed_state_id, outcome=outcome, before_label=before,
                            after_label=after, reason=reason, now=now)
    return next(r for r in get_run(run_id)["results"] if r["target_agent_id"] == target["agent_id"])
...
def export_migration(session: str) -> dict:
    ...
    ownership = [{"target_session": r["session"], "label": r["label"],
                 ...
                 for r in store.ownership_export_rows(a["agent_id"])]
    receipts = [{"target_session": r["target_session"], "before": r["before_label"],
                ...
                for r in store.receipt_export_rows(a["agent_id"])]
...
def import_pilot(session: str, expected_revision: int, expected_agent_id: str,
    ...
    with _write() as c:
        previous = store.pilot_import_row(import_id, c)
        if previous:
        ...
        event_attachments = store.active_attachment_rows(a["agent_id"], c, trigger_id="agent-work-completed")
        ...
        for value in ownership:
            ...
            owner = store.label_ownership_row(target["agent_id"], c)
            if owner and owner["owner_agent_id"] != a["agent_id"]:
        ...
        store.set_config_generation(c, a["agent_id"], generation, now)
        _save_progress(c, attachment["attachment_id"], generation, normalized)
        import_run_id = f"pilot-{import_id[:32]}"
        store.insert_run(c, run_id=import_run_id, agent_id=a["agent_id"], session=a["session"],
                         attachment_id=attachment["attachment_id"], generation=generation, trace_id=import_run_id,
                         status="completed", outcome="skipped", candidates=[], created_at=now, finished_at=now)
        for index, value in enumerate(receipts):
            target = targets[value["target_session"]]
            receipt_run = f"{import_run_id}-{index}"
            timestamp = _timestamp(value["at"])
            store.insert_run(c, run_id=receipt_run, agent_id=a["agent_id"], session=a["session"],
                             attachment_id=attachment["attachment_id"], generation=generation, trace_id=receipt_run,
                             status="completed", outcome=value["outcome"], candidates=[],
                             configuration={"source": "pilot", "import_id": import_id, "source_batch_id": value.get("batch_id", ""),
                                            "source_outcome": value.get("source_outcome", value["outcome"])},
                             created_at=timestamp, finished_at=timestamp)
            store.insert_effect(c, run_id=receipt_run, target_agent_id=target["agent_id"], target_session=target["session"],
                                observed_state_id=int(value.get("source_state_id") or 0), outcome=value["outcome"],
                                before_label=value.get("before", ""), after_label=value.get("after", ""),
                                reason=value.get("reason", ""), now=timestamp)
        for value in ownership:
            target = targets[value["target_session"]]
            current_state_id = store.latest_state_id(target["agent_id"], c)
            valid_until = now + LABEL_MAX_AGE_MS if current_state_id == value.get("source_state_id") else now
            store.upsert_label_ownership(c, target_agent_id=target["agent_id"], owner_agent_id=a["agent_id"], run_id=import_run_id,
                                         label=value["label"], task_signature=_task_signature(c, target["agent_id"]),
                                         valid_until=valid_until, now=now)
        store.record_pilot_import(c, import_id, a["agent_id"], now, digest)
    return get(session)


def _creation_result(c, row) -> dict:
    """Only a transactionally linked ID proves this request created an agent."""
    occupant = store.session_occupant(row["session"], c)
...
def begin_creation(request_id: str, payload: dict) -> dict:
    ...
    with _write() as c:
        existing = store.creation_request_row(request_id, c)
        if existing:
        ...
        for _ in range(10):
            ...
            if not agents.session_exists(session) and not store.creation_session_reserved(session, c):
                break
        else:
            raise JanitorError("Could not reserve a fresh session", 409, "creation_conflict")
        store.insert_creation_request(c, request_id=request_id, payload_json=encoded, payload_sha256=digest,
                                      identity=identity, session=session, now=db.now_ms())
        return _creation_result(c, store.creation_request_row(request_id, c))
...
def create_reserved_agent(request_id: str, *, persona: str, voice_id: str, cwd: str,
    ...
    """
    with _write() as c:
        row = store.creation_request_row(request_id, c)
        if not row:
...
def complete_creation(request_id: str, session: str) -> dict:
    ...
    with _write() as c:
        ...
        store.complete_creation_request(c, request_id, configured, db.now_ms())
        return configured
