...
def add_parsers(sub, handler) -> None:
    ...
    for name in ("inspect", "runs", "configure", "attach", "enable", "pause", "remove", "release", "export", "reset-defaults"):
        command = commands.add_parser(name)
        command.add_argument("session")
        command.set_defaults(func=handler)
        if name in {"configure", "attach", "enable", "pause", "remove", "release", "reset-defaults"}:
